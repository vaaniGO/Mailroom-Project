...
app.get('/backup', (req, res) => {
  ...
  const startDate = req.query.startDate || null; // Optional: Start date for range
  const endDate = req.query.endDate || null; // Optional: End date for range
...
app.post('/backup', (req, res) => {
  const { selectedDate, startDate, endDate } = req.body;
  ...
  if (startDate && endDate) {
    // Date range mode
    query = 'SELECT * FROM packages WHERE DATE(timestamp) BETWEEN ? AND ?';
    queryParams = [startDate, endDate];
  }
  else if (selectedDate) {
    // Single date mode
    query = 'SELECT * FROM packages WHERE DATE(timestamp) = ?';
    ...
    return res.render('backup', {
      ...
      endDate,
      packages: [],
      error: "Please select a valid date or date range",
      dateMode: 'err'
    });
  }
